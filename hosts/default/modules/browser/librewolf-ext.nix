    ...
    case $1 in
      windowtitle*)
        ...
        if [[ "$window_title" == *"(Bitwarden Password Manager) - Bitwarden"* ]]; then
          ...
          hyprctl --batch "dispatch togglefloating address:0x$window_id ; dispatch resizewindowpixel exact 500 800,address:0x$window_id ; dispatch movewindow address:0x$window_id,center"
        fi
        ;;
    esac
  }
